...
| chromadb | 向量数据库 |
| openai (或 sentence-transformers) | Embedding 生成 |
| tiktoken | Token 计数 |
...
class ShortTermMemory:
    ...
    def get_context(self) -> list[BaseMessage]:
        """返回给 LLM 的上下文：摘要 + 最近的对话"""
        result = []
        if self.summary:
            result.append(SystemMessage(content=f"[Previous context summary]: {self.summary}"))
...
async def compress_context(messages: list, existing_summary: str, 
    ...
    middle_msgs = messages[len(system_msgs):-20]
    ...
    if new_summary:
        compressed.append(SystemMessage(content=f"[Context]: {new_summary}"))
