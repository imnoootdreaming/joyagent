...
def _ensure_list(emb):
    """
    sentence-transformers 返回 numpy.ndarray；ChromaDB 和所有下游代码
    期望 Python list。此函数统一转换，避免 numpy bool 歧义错误。
    """
    try:
        return emb.tolist()  # numpy array
    except AttributeError:
        return list(emb)     # 已经是 list 则复制一次，安全
...
class EmbeddingService:
    ...
    def embed(self, text: str) -> list[float]:
        ...
        try:
            ...
            embedding = _ensure_list(result[0])   # numpy array → list
        except Exception as e:
            raise RuntimeError(
    ...
    def embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        for chunk_start in range(0, len(raw_texts), self.MAX_BATCH_SIZE):
            ...
            # 转换为纯 Python list（sentence-transformers 返回 numpy array）
            all_new_embeddings.extend(_ensure_list(v) for v in chunk_result)
