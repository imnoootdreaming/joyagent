...
# tool_safety: 全局安全检查器 —— 在 execute_shell 执行前拦截危险命令
#   注册在 ToolStatsCollector 之前，确保安全检查先于统计记录
...
def register_all_tools():
    ...
    print(f"  [OK] Registered 2 hooks: SafetyCheckHook "
          f"(block_hard={tool_safety.block_hard}) + "
          f"ToolStatsCollector (log every {tool_stats.log_interval} calls)")
